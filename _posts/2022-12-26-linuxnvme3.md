...
pci.c 파일을 열어 **nvme_probe**() 함수를 찾습니다.
...
- **1 단계:** 반환 값이 mask인 pci_select_bars() 함수를 호출합니다. pci 장치의 헤더 구성 공간에는 6개의 32비트 Bar 레지스터(아래 그림 참조)가 있으므로 마크의 각 비트 값은 Bar 중 하나가 설정되었는지 여부를 나타냅니다.

![img](/assets/images/linuxnvme3-1.jpg)

- **2단계:** pci_request_selected_regions() 함수 호출. 이 함수의 매개변수 중 하나는 이전에 pci_select_bars() 함수를 호출하여 반환된 마스크 값으로 해당 bar를 유지하고 다른 사람이 사용하지 못하도록 하는 기능입니다. 

- **3단계:** ioremap() 함수를 호출합니다. Linux에서는 물리적 주소에 직접 액세스할 수 없으며 가상 주소에 매핑해야 하며 ioremap이 이러한 목적을 위한 것입니다. 매핑 후 dev->bar를 방문하여 nvme 장치에서 레지스터를 직접 작동할 수 있습니다. 그러나 코드에서는 pci_select_bars() 함수의 반환 값에 따라 매핑할 막대를 결정하지 않고 bar0을 직접 매핑합니다. nvme 프로토콜이 bar0을 메모리 맵의 기본 주소로 지정하기 때문입니다.
...
위의 코드에서 nvme_setup_prp_pools() 함수는 주로 두 개의 dma 풀을 생성한 다음 다른 dma 함수를 통해 dma 풀에서 메모리를 가져올 수 있음을 이해할 수 있습니다. prp_small_pool은 블록 크기가 256바이트인 메모리를 제공하고, prp_page_pool은 주로 다양한 길이의 prp 목록을 최적화하기 위해 Page_Size(포맷 중에 결정됨, 예: 4KB) 블록 크기가 있는 메모리를 제공합니다.
...
위의 코드에서 nvme_init_ctrl의 기능은 주로 device_create_with_groups 기능을 호출하여 nvme0이라는 문자 장치를 생성하는 것임을 알 수 있습니다. 이 nvme0의 0은 nvme_set_instance에서 가져옵니다. 이 과정에서 ida_get_new를 통해 고유 인덱스 값을 얻는다.
...
nvme_rest_work는 매우 긴 작업 흐름으로 많은 콘텐츠가 포함되어 다음에 분석 하겠습니다.
