...
NVMe SSD는 PCIe 장치이므로 이러한 유형의 장치는 어떻게 식별할까?
...
Linux 커널 NVMe 드라이버에서는 첫 번째 , **SPDK에서는 두 번째 방법**을 사용합니다.
...
**클래스 코드(0x010802)**는 NVMe Spec. Rev. 1.4c Chap2 PCI Header에 다음과 같이 정의되어 있습니다 .
...
새로운 언어나 개발 키트를 배우기 시작할 때 가장 첫번째 예제는 "Hello World" 입니다. SPDK도 예외는 아닙니다. hello_world.c에서 시작하여 main()이 SPDK/NVMe 드라이버 API를 사용하는 방법을 살펴보고 NVMe SSD 사용의 주요 논리를 발견하는 데 도움을 줍니다.
...
- **003** 에서 spdk_nvme_probe() 함수를 호출하여NVMe SSD 장치를 능동적으로 검색합니다. 분명히 다음에 분석할 핵심 기능은 spdk_nvme_probe() 입니다 .
...
- 질문 1: 각 NVMe 솔리드 스테이트 드라이브에는 컨트롤러(Controller)가 있으므로 발견된 모든 NVMe 솔리드 스테이트 드라이브(즉, NVMe 컨트롤러)는 어떻게 함께 구성됩니까?
