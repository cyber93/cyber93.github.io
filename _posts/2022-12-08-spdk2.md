...
설명: 기본적으로 사이트 원문을 번역하였으나, 이해도를 높이기 위해 이미지는 일부 적합한 것으로 교체 하였음.
```
...
![Introduction to the Storage Performance Development Kit (SPDK)](./_media/spdk1.png)
...
SPDK는 어떻게 동작할까? 극한의 성능을 달성하기 위해 **사용자 모드** 실행 과 **[폴링 모드](http://doc.dpdk.org/guides/prog_guide/poll_mode_drv.html)**의 두 가지 핵심 기술이 사용됩니다 . 이 두 가지 소프트웨어 엔지니어링 용어를 자세히 살펴보겠습니다.
...
![Introduction to the Storage Performance Development Kit (SPDK)](./_media/spdk2.png)

#### **Hardware Drivers**

**NVMe 드라이버**: SPDK의 기본 구성 요소인 이 고도로 최적화된 잠금 없는 드라이버는 비교할 수 없는 확장성, 효율성 및 성능을 제공합니다 .

**인텔® QuickData 기술**: 인텔® I/O 가속 기술(인텔® IOAT)이라고도 하는 인텔® 제온® 프로세서 기반 플랫폼에 내장된 오프로드 복사 엔진입니다. 사용자 공간 액세스를 제공함으로써 DMA 데이터 이동에 대한 임계값이 줄어들어 작은 크기의 I/O 또는 NTB(Non-Transparent Bridge)에 대한 활용도가 높아집니다.
