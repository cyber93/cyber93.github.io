...
멀티코어(SMP) 멀티쓰레딩의 경우 코드리뷰를 아무리 해도 Memory Barrier(MB) 관련 버그를 찾는 것이 불가능하기 때문에 **CPU가 순서 없이 실행되는 것을 모르면 악몽이 될 것이다.** 

Memory Barrier은 두 가지 범주로 나뉩니다.

- 컴파일과 관련된 MB: 컴파일러에게 나를 최적화하지 말라고 지시하세요. 저는 필요없어요.
...
SPDK에서는 컴파일과 관련된 메모리 배리어 뿐만 아니라 CPU와 관련된 메모리 배리어도 구현되어 있다. 그러나 CPU관련 MB에서는 Read memory barrier가 구현되지 않는다.

<br>

## 3. DPDK의 Memory Barrier

DPDK에서는 x86, ARM 및 PowerPC의 세 가지 플랫폼을 지원하기 때문에 MB 구현이 조금 더 복잡합니다. x86을 예로 들면 코드 구현은 다음과 같습니다.
...
그래서 디스어셈블하고 gcc의 __sync_synchronize()에서 무슨 일이 일어나는지 봅시다.
